...
<!--         <button type="submit" name="btnService" class="btn nav-link bg-primary ">Username: <?php echo $myuser ?></button>
 -->    <button type="submit" name="btnService" class="btn nav-link bg-gradient-primary font-monospace ">SERVICE INFO</button>
        ...
        <button type="submit" name="btnResult" class="btn nav-link bg-gradient-primary ">ACADEMIC RESULT</button>

        <button type="submit" name="btnTourService" class="btn nav-link bg-gradient-primary ">TOUR SERVICE</button>
  ...
  <div id="content-wrapper"> 
    <nav class="navbar bg-white topbar navbar-fixed-top mb-4 shadow navbar-fixed-top static-top ">
      <h4 class="col-xl-6 col-md-8 col-sm-6 col-xs-4 text-primary"> Dhaka International University</h4>
            ...
            <div class='col-xl-6 col-lg-6 col-sm-12'>
                <div class='card shadow mb-5'>
                    <div class='card-header py-3 d-flex flex-row align-items-center justify-content-between bg-primary text-white'>
                        <h6 class='m-0 font-weight-bold'>TOUR SERVICE</h6>
                    </div>
                    <?php
                    // Display pending tour requests
                    $query = "SELECT * FROM tours WHERE status = 'pending'";
                    $result = mysqli_query($connection, $query);

                    echo "<div class='card-body'>";

                    if (mysqli_num_rows($result) > 0) {
                        echo "<h5 class='card-title'>Pending Tour Requests</h5>";

                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<p class='card-text'>{$row['student_username']} wants to join the tour event '{$row['event_name']}' on {$row['event_date']}.</p>";
                            echo "<form method='post' action='process_tour_request.php'>";
                            echo "<input type='hidden' name='tour_id' value='{$row['id']}'>";
                            echo "<button type='submit' name='btnAdmitTour' class='btn btn-success mr-2'>Admit</button>";
                            echo "<button type='submit' name='btnCancelTour' class='btn btn-danger'>Cancel</button>";
                            echo "</form>";
                        }
                    } else {
                        echo "<p class='card-text'>No pending tour requests.</p>";
                    }
